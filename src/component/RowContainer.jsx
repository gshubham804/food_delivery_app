...
<div
ref={rowContainer}
className={`w-full gap-3 flex scroll-smooth items-center my-12
 ${flag?"overflow-x-scroll scrollbar-none":
"overflow-x-hidden flex-wrap justify-center"}`}>
{
   data && data.length>0 ? data.map((item)=>(
        <div key={item?.id} className='w-275
h-[175px] min-w-[275px] md:w-300 md:min-w-[300px] bg-cardOverlay rounded-lg py-2 px-4 my-12 
...
<motion.div whileTap={{scale:0.75}} className='w-8 h-8
 rounded-full bg-red-600 flex items-center
...
font-semibold'>
    ...
    justify-center'>
      ...
      h-340 "/>
        ...
        font-semibold my-4'>Items Not Available</p>
        </div>}
</div>
  )
}
